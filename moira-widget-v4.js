document.addEventListener("DOMContentLoaded", function() {
    ...
    const scriptTag = document.querySelector('script[src*="https://cdn.jsdelivr.net/gh/MoiraAI2024/widget@main/moira-widget-v4.js"]');
    ...
    const moiraStyles = `
        ...
        #moira-widget #retry-message, #moira-widget #time-up-message {
            position: absolute;
            top: 50%;
            left: 50%;
            transform: translate(-50%, -50%);
        ...
        #moira-widget .moira-feedback {
            color: var(--widget-text-color);
            background-color: var(--widget-container-bg);
            border-radius: 8px;
            padding: 15px;
            margin-top: 20px;
            text-align: left;
        ...
        #moira-widget .moira-feedback p {
            margin: 0 0 5px 0;
        }

        #moira-widget .moira-feedback p:last-child {
            margin-bottom: 0;
        }
    `;
    ...
    const timeUpMessageDiv = document.createElement('div');
    timeUpMessageDiv.id = 'time-up-message';
    timeUpMessageDiv.textContent = "Time's up!";
    ...
    widgetContainer.appendChild(timeUpMessageDiv);
    moiraWidget.appendChild(widgetContainer);

    // Create feedback containers and append them to the main widget div
    const feedbackContainerGr = document.createElement('div');
    feedbackContainerGr.id = 'moira-feedback-gr';
    feedbackContainerGr.className = 'moira-feedback';
    moiraWidget.appendChild(feedbackContainerGr);

    const feedbackContainerEn = document.createElement('div');
    feedbackContainerEn.id = 'moira-feedback-en';
    feedbackContainerEn.className = 'moira-feedback';
    moiraWidget.appendChild(feedbackContainerEn);
    ...
    async function startRecording() {
        // Clear previous feedback before starting
        const feedbackGr = widget.querySelector('#moira-feedback-gr');
        const feedbackEn = widget.querySelector('#moira-feedback-en');
        const timeUpMessage = widget.querySelector('#time-up-message');
        if (timeUpMessage) {
            timeUpMessage.style.display = 'none';
        }
        if (feedbackGr) {
            feedbackGr.style.display = 'none';
            feedbackGr.innerHTML = '';
        }
        if (feedbackEn) {
            feedbackEn.style.display = 'none';
            feedbackEn.innerHTML = '';
        }
    ...
    async function sendAudioToApi(blob) {
        ...
        try {
            ...
            if (data && data.Audio) {
                ...
                if (feedbackGr && data.MoiraAI) {
                    feedbackGr.innerHTML = `<p><strong>Ελληνικά:</strong></p><p>${data.MoiraAI}</p>`;
                    feedbackGr.style.display = 'block';
                }
                if (feedbackEn && data.MoiraAI_English) {
                    feedbackEn.innerHTML = `<p><strong>English:</strong></p><p>${data.MoiraAI_English}</p>`;
                    feedbackEn.style.display = 'block';
                }
            } else {
